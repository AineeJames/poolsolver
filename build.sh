...
if [[ $? == 0 ]]; then
	...
	if [[ "$1" = "run" ]]; then
		...
		emcc -o html_build/pool.html src/*.c -Os -g3 -DNO_THREADS -Wall html_build/libraylib.a -Ihtmlbuild/ -I/usr/include/ -Lhtmlbuild/ -s ASYNCIFY -s USE_GLFW=3 -DPLATFORM_WEB --preload-file assets $(pkg-config --cflags raylib)
	fi
else
